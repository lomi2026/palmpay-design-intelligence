## 2026-09-18 标签恢复、默认启用与详情外链已上线

- 版本 `1898a6b` 已同步到 `main` 与 `codex/v1-project-handoff`：删除后的同名标签可恢复，新增或恢复标签默认启用；设计资产与 AI 案例详情增加安全校验后的外部打开入口；收藏激活态统一为灰色底、黑色实心心形图标。
- 发布前前端 69 项测试、生产构建和后端 79 项测试通过；另有 15 项需要独立测试数据库的集成用例按环境规则跳过。
- GitHub Vercel 状态为成功，正式前端经 Vercel 边缘节点返回 HTTP 200；Render API 分支已同步，正式健康检查返回 HTTP 200。当前网络对默认 Vercel 域名的普通 DNS 请求仍可能超时，属于网络解析链路问题，不是本次代码发布失败。
