...
- F-01 local implementation is complete: the four dedicated editors now persist the full P0 structured field contracts; incomplete drafts remain recoverable, while review submission and publication independently invoke the same server-side type-specific completeness validator and return concrete missing-field labels. Unit fixtures cover passing and missing-field cases for design assets, AI Skills, AI cases and AI projects. External test-environment browser verification remains required before F-01 is accepted as deployed.
- F-02 local implementation is complete: each formal published detail page renders ready attachments from its immutable current version and requests a short-lived protected download URL only after a user action. Download authorization now requires an organization-scoped published content binding plus the same visibility boundary as the content itself; integration evidence confirms a permitted organization attachment download and a 403 response for another owner’s restricted attachment. External test-environment browser verification remains required before F-02 is accepted as deployed.
...
- F-06 browser acceptance was initiated on 2026-07-18: the public Vercel test home and test-login route responded, and the three role tabs can be logged in. The browser shares one cookie scope across those tabs, however, so server-rendered navigation resolves to the most recently authenticated identity rather than the role originally shown in a tab. Browser-control attempts to take over or fill the login form also time out. No acceptance content or files were written. Resume using isolated browser profiles/contexts or responsive browser control, then perform the contributor/reviewer/administrator workflow.
- Release commit `a034c73` publishes the locally verified F-01 through F-05 implementation to `codex/v1-project-handoff`. GitHub reports the Vercel deployment complete, the public Render `/api/health` endpoint returns `{"status":"ok"}`, and the external administrator workspace loads successfully. This is deployment evidence, not a substitute for the still-open F-06 three-role browser workflow.
...
1. Use isolated browser profiles/contexts (or restore responsive browser control with separate cookie scopes), then add and execute F-06 deployed browser evidence for contributor, reviewer and administrator flows, including upload and signed download; externally verify F-01 through F-05 with the deployed role workflow.
2. Record the resulting acceptance evidence, blockers and any necessary release-environment configuration changes without weakening formal RBAC, versioning or audit records.
...
- Release commit `a034c73` is pushed to `codex/v1-project-handoff` and has a successful Vercel deployment plus a healthy Render API response. GitHub Pages remains enabled but serves the legacy static `main`-branch root; it is not the formal V1 deployment route. `docs/11-RELEASE-READINESS.md` records the remaining production-hosting, database, SSO, domain and CI/CD prerequisites.
