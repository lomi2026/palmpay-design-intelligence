...
By explicit user decision on 2026-08-30, the isolated external-test login no longer asks for or validates the shared test access code. Email-only login still requires a pre-provisioned active database user and issues the same short-lived HMAC-signed session, so disabled-user checks and RBAC remain authoritative; the production OIDC/SSO boundary is unchanged. The login page now starts a health request when opened to wake the sleeping Render API and shows connection and form-pending feedback instead of leaving the user with an apparently unresponsive submit action. Web typecheck, lint, 24 tests and the 23-page production build pass; API typecheck, lint and the 48-test suite pass with 14 database-dependent integration cases skipped because the database URL was not supplied to that test process. Local production rendering confirms that the access-code field is absent and the warm-up status is announced. Commit `7a382cc` was pushed to the external-test branch and Vercel deployed the updated Web successfully. The corresponding Render deployment `dep-daa381vlk1mc738gnmag` failed because the expired free PostgreSQL instance was unreachable (`P1001`). The attempted minimum paid PostgreSQL upgrade was withdrawn after the user confirmed that a monthly database charge is not available. By subsequent user decision, the existing free API is being reconnected to a newly provisioned free PostgreSQL instance (`palmpay-design-hub-test-db-v2`); this restores a no-monthly-charge acceptance route but retains Render's 30-day free-database expiry constraint. A temporary Blueprint Docker command runs the idempotent migration, seed, test-account bootstrap and all three v9-1 imports on the first successful connection to the replacement database; it must be removed after initialization verification so routine free-instance cold starts return to migration-only startup.
