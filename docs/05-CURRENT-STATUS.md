...
- The management-center tab performance defect has been corrected: each tab now loads only its own authorized data (one or two endpoints rather than all eight administration datasets), inactive tab links no longer trigger automatic background prefetches, and the current-user/auth-header reads are request-memoized across the workspace shell and page. A workspace-wide route review found no other query-tab surface that eagerly loads unrelated administration datasets.
- The external-test workspace now keeps recently visited dynamic route segments in the browser for 20 seconds and prefetches sidebar routes only after pointer or keyboard intent. Large catalog/detail-link sets no longer automatically prefetch all visible records. This removes repeated Vercel-to-Render reads during normal back-and-forth navigation while retaining server-side RBAC enforcement; Render free-instance cold starts remain a hosting limitation.
