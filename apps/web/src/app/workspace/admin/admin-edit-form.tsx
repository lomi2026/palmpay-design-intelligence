...
import { startTransition, useActionState, useEffect, type ComponentProps } from 'react';
...
export function AdminEditForm({ action, ...props }: AdminEditFormProps) {
  ...
  const [result, formAction] = useActionState<AdminSaveResult, FormData>(
    async (_previous: AdminSaveResult, formData: FormData): Promise<AdminSaveResult> => {
      try {
        ...
        return { status: 'error', message: '连接中断，保存结果暂未确认。请刷新核对状态后再重试。' };
      }
    },
    { status: 'idle', message: '' },
  );
