  ...
  return (
    <div className="v9-source-home min-h-screen bg-[var(--v9-bg)] text-[var(--v9-text)]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[224px] border-r border-[var(--v9-line)] bg-[var(--v9-sidebar)] md:flex md:flex-col">
        <Link href="/" className="flex h-16 items-center gap-3 border-b border-white/[.11] px-5"><Image src="/v9-1/assets/nav-logo-20260710.png" alt="PalmPay Design" width={31} height={31} className="size-[31px] object-contain" /><span className="text-[13px] font-bold">PalmPay Design</span></Link>
        ...
        <div className="m-3 rounded-xl bg-white/[.055] p-3"><div className="flex items-center justify-between"><strong className="text-[11px]">Beta 1.0</strong><Badge variant="outline" className="border-white/[.1] text-[9px] text-white/65">内部</Badge></div><p className="mt-2 text-[11px] leading-4 text-white/45">内容与数据为内部试运行口径，更新于 2026.07。</p></div>
      </aside>
      <div className="md:pl-[224px]">
        <header className="sticky top-0 z-20 flex h-16 items-center border-b border-[var(--v9-line)] bg-[color-mix(in_srgb,var(--v9-bg)_95%,transparent)] px-4 backdrop-blur-xl md:px-8"><WorkspaceMobileNavigation {...navigationProps} /><WorkspaceBreadcrumb /><WorkspaceSearchShortcut /><div className="ml-auto flex items-center gap-2"><V9ThemeToggle /><NotificationBadge initialUnreadCount={notifications.unreadCount} key={`notifications-${notifications.unreadCount}`} /><WorkspaceAccountMenu name={user.name} roleLabel={roleLabel} /></div></header>
        {children}
      </div>
    </div>
  );
