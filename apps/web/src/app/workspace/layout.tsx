...
export default async function WorkspaceLayout({ children }: Readonly<{ children: React.ReactNode }>) {
  ...
  return (
    <FavoriteProvider ids={favorites.items.map(item=>item.content.id)}><div className="workspace-shell v9-source-home min-h-screen bg-[var(--v9-bg)] text-[var(--v9-text)]">
      <aside className="fixed inset-y-0 left-0 z-30 hidden w-[224px] border-r border-[var(--v9-line)] bg-[var(--v9-sidebar)] md:flex md:flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-2.5">
          <Button asChild variant="outline" className="mb-6 h-10 w-full justify-start rounded-[10px] border-white/[.15] bg-transparent px-3 text-[13px] text-white hover:bg-white/[.06] hover:text-white"><Link href="/"><span className="mr-1">←</span> 返回平台首页</Link></Button>
          <WorkspaceSidebarNavigation {...navigationProps} />
        </div>
      </aside>
      <div className="md:pl-[224px]">
        <header className="sticky top-0 z-20 flex h-16 items-center border-b border-[var(--v9-line)] bg-[color-mix(in_srgb,var(--v9-bg)_95%,transparent)] px-4 backdrop-blur-xl md:px-8"><WorkspaceMobileNavigation {...navigationProps} /><WorkspaceBreadcrumb /><WorkspaceSearchShortcut /><div className="ml-auto flex items-center gap-2"><Button asChild variant="outline" size="icon-sm" className="size-10 rounded-[10px] border-border bg-background text-foreground hover:bg-muted hover:text-foreground"><Link href="/workspace/favorites" aria-label="收藏与浏览" title="收藏与浏览"><Heart className="size-4" /></Link></Button><V9ThemeToggle /><NotificationBadge initialUnreadCount={notifications.unreadCount} key={`notifications-${notifications.unreadCount}`} /><WorkspaceAccountMenu email={user.email} name={user.name} roleLabel={roleLabel} /></div></header>
        {children}
      </div>
