        ...
        <section className="mt-5 overflow-hidden rounded-[28px] border border-white/[.1] bg-[linear-gradient(145deg,#111,#171717)]">
          <div className="grid lg:grid-cols-[1.18fr_.82fr]">
            <div className="p-7 md:p-12">
              <div className="min-w-0"><p className="text-[11px] font-bold tracking-[.16em] text-white/45">{skill.category?.name ?? 'AI 能力'} · 可复用方法</p><h1 className="mt-3 max-w-[760px] break-words text-[42px] font-semibold leading-[1.02] tracking-[-.055em] text-white md:text-[60px]">{skill.title}</h1></div>
              <div className="mt-6 flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
                <div className="flex min-w-0 flex-wrap items-center gap-2"><FavoriteControl contentId={skill.id} returnTo={`/workspace/ai-skills/${skill.slug}`} /><ContentEngagementLinks contentId={skill.id} /></div>
                {canEdit || canArchive || canUnpublish ? <div className="flex min-w-0 flex-wrap items-start gap-2 xl:justify-end">{canEdit ? <PublishedEdit contentId={skill.id} /> : null}<ContentLifecycle canArchive={canArchive} canUnpublish={canUnpublish} contentId={skill.id} /></div> : null}
              </div>
