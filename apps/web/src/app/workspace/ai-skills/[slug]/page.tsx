        ...
        <section className="mt-5 overflow-hidden rounded-[28px] border border-white/[.1] bg-[linear-gradient(145deg,#111,#171717)]">
          <div className="grid lg:grid-cols-[1.18fr_.82fr]">
            <div className="p-7 md:p-12">
              <div className="flex flex-wrap items-start justify-between gap-5"><div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-white/45">{skill.category?.name ?? 'AI capability'} · Reusable method</p><h1 className="mt-3 max-w-[760px] text-[42px] font-semibold leading-[1.02] tracking-[-.055em] text-white md:text-[60px]">{skill.title}</h1></div><div className="flex flex-wrap justify-end gap-2"><FavoriteControl contentId={skill.id} returnTo={`/workspace/ai-skills/${skill.slug}`} /><ContentEngagementLinks contentId={skill.id} />{canEdit ? <PublishedEdit contentId={skill.id} /> : null}<ContentLifecycle canArchive={canArchive} canUnpublish={canUnpublish} contentId={skill.id} /></div></div>
