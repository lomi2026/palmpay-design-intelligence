...
function MetricList({
  ...
  return (
    <section data-card-surface="" className="rounded-[18px] border border-white/[.1] bg-[#111112] p-6 sm:p-6">
...
export default async function InsightsPage() {
  ...
  return (
    <main className="mx-auto min-h-full w-full max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <section className="flex flex-col justify-between gap-6 border-b border-white/[.1] pb-7 md:flex-row md:items-end">
      ...
      <section className="content-metric-cards mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
        <article data-card-surface="" className="rounded-[18px] border border-white/[.1] bg-white/[.03] p-6"><span className="grid size-9 place-items-center rounded-xl bg-white/[.06]"><BarChart3 className="size-4 text-white/70" /></span><strong className="mt-6 block text-[37px] font-semibold leading-none tracking-[-.06em] text-white">{contentTotal}</strong><p className="mt-2 text-[13px] font-medium text-white">正式目录内容</p><p className="mt-1 text-[11px] text-white/45">按当前内容类型分布汇总</p></article>
        <article data-card-surface="" className="rounded-[18px] border border-white/[.1] bg-white/[.03] p-6"><span className="grid size-9 place-items-center rounded-xl bg-white/[.06]"><ArrowUpRight className="size-4 text-white/70" /></span><strong className="mt-6 block text-[37px] font-semibold leading-none tracking-[-.06em] text-white">{topSearchTotal}</strong><p className="mt-2 text-[13px] font-medium text-white">有效搜索信号</p><p className="mt-1 text-[11px] text-white/45">可用于发现高频能力需求</p></article>
        <article data-card-surface="" className="rounded-[18px] border border-white/[.1] bg-white/[.03] p-6"><span className="grid size-9 place-items-center rounded-xl bg-white/[.06]"><SearchX className="size-4 text-white/70" /></span><strong className="mt-6 block text-[37px] font-semibold leading-none tracking-[-.06em] text-white">{noResultTotal}</strong><p className="mt-2 text-[13px] font-medium text-white">无结果搜索</p><p className="mt-1 text-[11px] text-white/45">提示目录和标签可能存在缺口</p></article>
      </section>
