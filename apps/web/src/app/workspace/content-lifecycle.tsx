...
export function ContentLifecycle({ contentId, canArchive, canUnpublish }: { contentId: string; canArchive: boolean; canUnpublish: boolean }) {
  ...
  return <form action={action} className="flex max-w-[250px] flex-col items-end gap-1.5"><input name="id" type="hidden" value={contentId} />
    <div className="flex flex-wrap justify-end gap-2">
      {canUnpublish ? <Button variant="outline" className="h-9 rounded-[10px] border-amber-400/40 bg-black/[.16] px-3 text-[12px] text-amber-200 hover:bg-amber-400/10 hover:text-amber-100" disabled={pending} name="operation" type="submit" value="unpublish">{pending ? '处理中…' : '下架内容'}</Button> : null}
    ...
    <span className="text-right text-[10px] leading-4 text-white/40">下架或归档后，内容将不再出现在公开目录中</span>
    {state.error ? <p className="max-w-64 text-right text-[11px] text-red-400">{state.error}</p> : null}
  </form>;
}
