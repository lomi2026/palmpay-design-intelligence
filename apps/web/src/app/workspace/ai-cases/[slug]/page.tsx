...
export default async function AICaseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="min-h-full bg-[radial-gradient(circle_at_8%_0%,rgba(255,255,255,.065),transparent_30%),#090909] px-4 py-5 md:px-6"><div className="mx-auto max-w-[1232px]">
      ...
      <section className="mt-5"><div className="mb-8 flex flex-wrap items-end justify-between gap-5"><div><p className="text-[12px] font-bold uppercase tracking-[.16em] text-white/45">Verified AI practice</p><h1 className="mt-2 text-[42px] font-semibold tracking-[-.055em] text-white md:text-[58px]">{item.title}</h1><p className="mt-3 max-w-[760px] text-[15px] leading-7 text-white/55">{item.summary ?? detail?.background ?? '暂无案例说明。'}</p></div><div className="flex flex-wrap justify-end gap-2"><FavoriteControl contentId={item.id} returnTo={`/workspace/ai-cases/${item.slug}`} /><ContentEngagementLinks contentId={item.id} />{canEdit ? <PublishedEdit contentId={item.id} /> : null}<ContentLifecycle canArchive={canArchive} canUnpublish={canUnpublish} contentId={item.id} /></div></div>
