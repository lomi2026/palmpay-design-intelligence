...
export default async function AICaseDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-6 py-7 md:px-10">
      ...
      <header className="mt-6 border-b border-[var(--border)] pb-7"><p className="text-xs tracking-[0.18em] text-neutral-500">{item.category?.name ?? 'PRACTICE EVIDENCE'}</p><h1 className="mt-3 text-3xl font-semibold leading-tight">{item.title}</h1><p className="mt-4 max-w-3xl text-base leading-7 text-neutral-400">{item.summary ?? '暂无说明'}</p><div className="mt-5 flex flex-wrap gap-x-6 gap-y-2 text-sm text-neutral-500"><span>状态：{verificationLabels[item.verificationStatus] ?? item.verificationStatus}</span><span>验证：{detail?.validationMethod ?? body.validation ?? '待补充'}</span><span>版本：{item.currentVersion?.versionLabel ?? 'v1'}</span></div></header>
