...
export default async function AICasesPage({ searchParams }: { searchParams: Promise<{ search?: string; categoryId?: string; tag?: string; verificationStatus?: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-[1440px] px-5 py-8 md:px-8 md:py-10">
      ...
      {cases.items.length ? <section className="mt-6 grid gap-4 md:grid-cols-2">{cases.items.map((item) => <Card key={item.id} className="group min-h-56 border border-[var(--v9-line)] bg-[var(--v9-panel)] py-5 shadow-none transition hover:-translate-y-0.5 hover:border-[var(--v9-line-strong)] hover:bg-[var(--v9-panel-2)]"><CardHeader><CardTitle className="mt-4 text-[19px] leading-7 text-[var(--v9-text)]"><Link className="transition group-hover:text-[var(--v9-muted)]" href={`/workspace/ai-cases/${item.slug}`} prefetch={false}>{item.title}</Link></CardTitle><p className="mt-2 line-clamp-3 text-sm leading-6 text-[var(--v9-copy)]">{item.summary ?? '暂无说明'}</p></CardHeader>{item.caseDetail?.metricName ? <CardContent className="mt-auto"><div className="border-t border-[var(--v9-line)] pt-4 text-sm text-[var(--v9-muted)]"><span className="mr-2 text-[10px] tracking-[.12em] text-[var(--v9-subtle)]">观察指标</span>{item.caseDetail?.metricName ?? '指标待验证'}</div></CardContent> : null}<CardFooter className="justify-between border-0 bg-transparent px-5 pb-5"><span className="text-xs text-[var(--v9-subtle)]">负责人 · {item.owner.name}</span><Link className="inline-flex items-center gap-1 text-sm text-[var(--v9-copy)] transition hover:text-[var(--v9-text)]" href={`/workspace/ai-cases/${item.slug}`} prefetch={false}>查看案例 <ArrowRight className="size-4" /></Link></CardFooter></Card>)}</section> : <WorkspaceEmptyState className="mt-6 py-16 text-center">没有找到可访问的 AI 案例。</WorkspaceEmptyState>}
    </main>
  );
}
