    ...
    <main className="mx-auto min-h-full w-full max-w-[1440px] px-4 py-6 sm:px-6 lg:px-8 lg:py-8">
      <section data-card-surface="" className="workspace-page-card relative overflow-hidden rounded-[24px] border border-white/[.12] bg-[#111112] px-6 py-7 sm:px-8 sm:py-9 lg:px-10 lg:py-11">
        ...
        <div className="relative grid gap-8 xl:grid-cols-[minmax(0,1fr)_410px] xl:items-end">
          ...
          <div data-card-surface="" className="rounded-[18px] border border-white/[.12] bg-black/[.24] p-6">
      ...
      <section className="content-metric-cards mt-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        {contentMetrics.map(([key, label, detail]) => (
          <article data-card-surface="" className="rounded-[18px] border border-white/[.1] bg-white/[.03] p-6" key={key}>
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1.2fr)_minmax(360px,.8fr)]">
        <div data-card-surface="" className="rounded-[20px] border border-white/[.1] bg-[#111112] p-6 sm:p-6">
          ...
          <div className="mt-6 grid gap-4 sm:grid-cols-2">
            {evidenceMetrics.map(([Icon, label, value, detail]) => <article data-card-surface="" className="rounded-[16px] border border-white/[.1] bg-white/[.025] p-4" key={label}><Icon className="size-4 text-white/70" /><strong className="mt-6 block text-[30px] font-semibold leading-none tracking-[-.05em] text-white">{value}</strong><p className="mt-2 text-[13px] font-medium text-white">{label}</p><p className="mt-1 text-[11px] leading-5 text-white/45">{detail}</p></article>)}
          </div>
        </div>
        <aside data-card-surface="" className="flex flex-col justify-between rounded-[20px] border border-[var(--v9-line-strong)] bg-card p-6 text-[var(--v9-text)] sm:p-6">
          <div>
      ...
      <section data-card-surface="" className="mt-6 flex flex-col justify-between gap-4 rounded-[18px] border border-white/[.1] bg-white/[.025] px-5 py-5 sm:flex-row sm:items-center sm:px-6"><Button asChild className="shrink-0 bg-white text-black hover:bg-white/90"><Link href="/workspace/ai-projects">查看 AI 项目库 <ChartNoAxesCombined className="size-4" /></Link></Button></section>
    </main>
  );
}
