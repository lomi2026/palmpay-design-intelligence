...
export default async function AIProjectDetailPage({
  ...
  const projectCode = detail?.projectCode ?? 'AI PROJECT';
  ...
  return (
    <main className="min-h-full bg-[radial-gradient(circle_at_8%_0%,rgba(255,255,255,.065),transparent_30%),#090909] px-4 py-5 md:px-6">
      <div className="mx-auto max-w-[1460px]">
        <Link className="inline-flex items-center gap-2 text-[12px] text-white/55 transition hover:text-white" href="/workspace/ai-projects"><ArrowLeft className="size-4" />返回 AI 项目库</Link>
        ...
        <section className="mt-5 overflow-hidden rounded-[30px] border border-white/[.1] bg-[linear-gradient(145deg,#111,#171717)] shadow-2xl shadow-black/25">
          <div className="grid min-h-[610px] lg:grid-cols-[1.35fr_.65fr]">
            <div className="p-7 md:p-12 lg:p-[58px] lg:pt-[68px]">
              <p className="flex items-center gap-2 text-[12px] font-bold uppercase tracking-[.16em] text-white/50"><span className="size-1.5 rounded-full bg-white" />{projectCode} · Design Intelligence Hub</p>
              <div className="mt-5 flex flex-wrap items-start justify-between gap-5"><h1 className="max-w-[1000px] text-[46px] font-semibold leading-[.98] tracking-[-.065em] text-white md:text-[clamp(46px,6.3vw,88px)]">{project.title}</h1><div className="flex flex-wrap justify-end gap-2"><FavoriteControl contentId={project.id} returnTo={`/workspace/ai-projects/${project.slug}`} /><ContentEngagementLinks contentId={project.id} />{canEdit ? <PublishedEdit contentId={project.id} /> : null}<ContentLifecycle canArchive={canArchive} canUnpublish={canUnpublish} contentId={project.id} /></div></div>
