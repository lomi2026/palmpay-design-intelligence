  ...
  return (
    <main className="min-h-full bg-[radial-gradient(circle_at_8%_0%,rgba(255,255,255,.065),transparent_30%),#090909] px-3 py-3 md:px-[22px] md:py-[22px]">
      <div className="mx-auto max-w-[1460px]">
        ...
        <section data-card-surface="" className="mt-6 overflow-hidden rounded-[30px] border border-white/[.1] bg-[linear-gradient(145deg,#111,#171717)] shadow-2xl shadow-black/25">
          <div className="block">
            <div className="p-4 md:p-6">
              ...
              <div className="mt-6 flex flex-wrap items-start justify-between gap-6"><h1 className="max-w-[1000px] break-words text-[44px] font-semibold leading-[.98] tracking-[-.065em] text-white md:text-[clamp(46px,6.2vw,86px)]">{project.title}</h1><DetailActions><FavoriteControl contentId={project.id} returnTo={`/workspace/ai-projects/${project.slug}`} /><ContentEngagementLinks contentId={project.id} />{canEdit || canArchive || canUnpublish ? <div className="flex w-full flex-wrap items-start gap-3 border-t border-[var(--v9-line)] pt-3">{canEdit ? <PublishedEdit contentId={project.id} /> : null}<ContentLifecycle canArchive={canArchive} canUnpublish={canUnpublish} contentId={project.id} /></div> : null}</DetailActions></div>
              <p className="mt-6 max-w-[840px] text-[16px] leading-8 text-white/55 md:text-[18px]">{projectSummary}</p>
              <div className="mt-9 grid gap-2.5 md:grid-cols-3"><div data-card-surface="" className="rounded-[15px] border border-white/[.1] bg-[#090909] p-4"><small className="block text-[11px] font-bold text-white/45">当前阶段</small><strong className="mt-1.5 block text-[16px] leading-6 text-white">{stage}</strong></div><div data-card-surface="" className="rounded-[15px] border border-white/[.1] bg-[#090909] p-4"><small className="block text-[11px] font-bold text-white/45">验证状态</small><strong className="mt-1.5 block text-[16px] leading-6 text-white">{verificationLabels[project.verificationStatus] ?? project.verificationStatus}</strong></div><div data-card-surface="" className="rounded-[15px] border border-white/[.1] bg-[#090909] p-4"><small className="block text-[11px] font-bold text-white/45">建议团队</small><strong className="mt-1.5 block text-[16px] leading-6 text-white">{detail?.suggestedOwnerTeam?.name ?? project.team.name}</strong></div></div>
