        ...
        <section data-card-surface="" className="project-detail-intro mt-6 rounded-[24px] bg-[var(--v9-panel)]">
          <div className="block">
            <div className="p-5 md:p-6">
              ...
              <div className="mt-3"><h1 className="max-w-[1000px] break-words text-[44px] font-semibold leading-[.98] tracking-[-.065em] text-white md:text-[clamp(46px,6.2vw,86px)]">{project.title}</h1><p className="mt-3 max-w-[960px] text-sm leading-7 text-[var(--v9-copy)]">{projectSummary}</p><div className="detail-toolbar mt-6 w-full">{body.source?.legacyProjectUrl ? <Button asChild><a href={body.source.legacyProjectUrl} target="_blank" rel="noopener noreferrer"><ExternalLink aria-hidden="true" />查看项目</a></Button> : null}<FavoriteControl size="md" contentId={project.id} returnTo={`/workspace/ai-projects/${project.slug}`} /><ContentEngagementLinks contentId={project.id} canonicalPath={`/workspace/ai-projects/${encodeURIComponent(project.slug)}`} />{canEdit ? <PublishedEdit contentId={project.id} /> : null}{canEdit || canArchive || canUnpublish ? <ManagementMenu><summary aria-label="更多管理操作"><MoreHorizontal size={18} /></summary><div><p>内容管理</p><div className="flex items-start gap-3"><ContentLifecycle canArchive={canArchive} canUnpublish={canUnpublish} contentId={project.id} />{canEdit ? <DeleteContentButton filled contentId={project.id} title={project.title} redirectTo="/workspace/ai-projects" className="h-9 px-3 text-[12px]" /> : null}</div></div></ManagementMenu> : null}</div></div>
