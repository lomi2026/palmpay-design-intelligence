...
export default async function AIProjectsPage({ searchParams }: { searchParams: Promise<{ search?: string; categoryId?: string; tag?: string; verificationStatus?: string }> }) {
  ...
  return (
    <main className="px-5 py-8 md:px-8 md:py-10">
      <header className="border-b border-white/[.1] pb-7">
        <div className="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
          ...
          <form className="flex w-full max-w-xl gap-2" method="get">
            ...
            {verificationStatus ? <input name="verificationStatus" type="hidden" value={verificationStatus} /> : null}
      ...
      <CatalogFilterControls contents={filterSource.items} filters={filters} pathname="/workspace/ai-projects" />
