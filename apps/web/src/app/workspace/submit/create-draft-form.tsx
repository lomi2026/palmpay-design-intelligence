    ...
    <form action={action} className="mt-6 grid gap-5 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5 md:p-6">
      <section className="grid gap-4 md:grid-cols-2">
        <label className="grid gap-1.5 text-sm text-neutral-300">内容类型
          <select className="h-9 rounded-md border border-[var(--border)] bg-black px-3 text-sm" name="contentType" value={contentType} onChange={(event) => setContentType(event.target.value as typeof contentType)}>
