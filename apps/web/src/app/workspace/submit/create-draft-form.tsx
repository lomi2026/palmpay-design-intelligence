...
export function CreateDraftForm({
  ...
  return (
    <form action={action} onResetCapture={(event) => { event.preventDefault(); event.stopPropagation(); }} className="composer-create mt-6 overflow-hidden rounded-[20px] border border-[var(--v9-line)] bg-[var(--v9-panel)]">
      <section className="border-b border-[var(--v9-line)] p-6 md:p-6">
        ...
        <div className="mt-6 grid gap-2 sm:grid-cols-2 xl:grid-cols-5">{[...contentTypes].sort((a,b) => ['DESIGN_ASSET','AI_TOOL','AI_SKILL','AI_PROJECT','AI_CASE'].indexOf(a.value)-['DESIGN_ASSET','AI_TOOL','AI_SKILL','AI_PROJECT','AI_CASE'].indexOf(b.value)).map(({ value, label, description, icon: Icon }) => <button aria-pressed={contentType === value} className={`rounded-[14px] border p-4 text-left text-[var(--v9-text)] transition focus-visible:outline-none focus-visible:ring-3 focus-visible:ring-[var(--v9-soft-hover)] ${contentType === value ? 'border-[var(--v9-status-accent-line)] bg-[var(--v9-status-accent-bg)]' : 'border-[var(--v9-line)] bg-[var(--v9-soft)] hover:border-[var(--v9-line-strong)] hover:bg-[var(--v9-soft-hover)]'}`} key={value} onClick={() => setContentType(value)} type="button"><Icon className={contentType === value ? 'size-4 text-[var(--v9-status-accent-text)]' : 'size-4 text-[var(--v9-muted)]'} /><strong className="mt-6 block text-[14px]">{label}</strong><span className="mt-1.5 block text-[11px] leading-5 text-[var(--v9-copy)]">{description}</span></button>)}</div>
      </section>
      <section className="grid gap-6 p-6 md:grid-cols-2 md:p-6">
        ...
        <p className="text-sm leading-6 text-[var(--v9-copy)] md:col-span-2">下一步完善使用方法和发布信息。现在只会创建私人草稿。</p>
      </section>
