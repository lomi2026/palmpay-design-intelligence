...
import { TaxonomyFields, type TaxonomyOptions, type TaxonomySelection } from '@/components/workspace/taxonomy-fields';

export type Draft = { taxonomy: TaxonomySelection; taxonomyOptions: TaxonomyOptions; id: string; status: string; contentType: 'DESIGN_ASSET' | 'AI_SKILL' | 'AI_CASE' | 'AI_PROJECT'; title: string; summary: string | null; draftVersion: { title: string; summary: string | null; body: unknown; changeSummary: string | null; versionNumber: number; versionStatus: string } | null; attachments: Array<{ id: string; fileId: string; file: { originalName: string; mimeType: string; sizeBytes: string } }> };
...
export function DraftEditor({ draft }: { draft: Draft }) {
  ...
  const clearScheduledSave = () => { if (saveTimer.current) clearTimeout(saveTimer.current); };
  useEffect(() => () => { if (saveTimer.current) clearTimeout(saveTimer.current); }, []);
  return <>
    <form action={action} onResetCapture={(event) => { event.preventDefault(); event.stopPropagation(); }} className="mt-6 grid gap-6 rounded-[24px] border border-white/[.1] bg-[#111] p-5 md:p-8" onInput={scheduleAutosave} onSubmit={clearScheduledSave} ref={formRef}><input name="id" type="hidden" value={draft.id} /><input name="contentType" type="hidden" value={draft.contentType} />
    ...
    <div className="grid gap-5 lg:grid-cols-[minmax(0,1.2fr)_minmax(320px,.8fr)]"><label className="grid gap-2 text-sm font-medium text-white/75">标题<Input className="h-12 border-white/[.14] bg-black/25 text-base text-white" defaultValue={draft.draftVersion?.title ?? draft.title} name="title" required /></label>
    ...
    <label className="grid gap-2 text-sm font-medium text-white/75">摘要<Textarea className="min-h-28 border-white/[.14] bg-black/25 text-white" defaultValue={draft.draftVersion?.summary ?? draft.summary ?? ''} name="summary" /></label>
    <TaxonomyFields options={draft.taxonomyOptions} initialSelection={draft.taxonomy} contentType={draft.contentType} onChange={scheduleAutosave} />
