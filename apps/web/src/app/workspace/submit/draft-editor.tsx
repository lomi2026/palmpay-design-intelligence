...
type Draft = { id: string; status: string; title: string; summary: string | null; draftVersion: { body: unknown; changeSummary: string | null; versionNumber: number } | null };
...
export function DraftEditor({ draft }: { draft: Draft }) {
  ...
  return <>
    <form action={action} className="mt-6 grid gap-5 rounded-lg border border-[var(--border)] bg-[var(--surface)] p-5 md:p-6"><input name="id" type="hidden" value={draft.id} />
    ...
    <label className="grid gap-1.5 text-sm text-neutral-300">结构化内容<textarea className="min-h-56 rounded-md border border-[var(--border)] bg-black px-3 py-2 font-mono text-xs leading-5" defaultValue={JSON.stringify(draft.draftVersion?.body ?? {}, null, 2)} name="body" spellCheck={false} /></label>
