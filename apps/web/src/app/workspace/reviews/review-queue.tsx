...
function ReviewCard({ review, reviewers, currentUserId, canPublish }: { review: Review; reviewers: Reviewer[]; currentUserId: string; canPublish: boolean }) {
  ...
  return <Card className="border border-white/10 bg-white/[0.035] py-5 shadow-none"><CardHeader>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs tracking-[0.14em] text-white/40">{review.content.contentType} · v{review.version.versionNumber}</p><CardTitle className="mt-1 text-lg text-white">{review.version.title}</CardTitle><p className="mt-2 text-sm text-white/50">投稿人：{review.submittedBy.name} · {new Date(review.submittedAt).toLocaleString('zh-CN')}</p></div><Badge variant="outline" className="border-white/15 text-white/70">{review.status}</Badge></div>
    ...
    {review.status === 'PENDING' ? <form action={assignAction} className="flex flex-wrap items-center gap-2 border-t border-white/10 pt-4"><input name="id" type="hidden" value={review.id} /><select className="h-8 rounded-lg border border-white/15 bg-black/25 px-3 text-sm text-white" defaultValue={review.assignedReviewer?.id ?? ''} name="reviewerId" required><option value="">选择审核人</option>{reviewers.filter((reviewer) => reviewer.id !== review.submittedBy.id).map((reviewer) => <option key={reviewer.id} value={reviewer.id}>{reviewer.name} · {reviewer.email}</option>)}</select><Button variant="outline" className="border-white/15 bg-transparent text-white hover:bg-white/10 hover:text-white" disabled={assigning} type="submit">分配</Button>{review.assignedReviewer ? <span className="text-xs text-white/40">当前：{review.assignedReviewer.name}</span> : null}</form> : null}
    {review.status === 'PENDING' && assignedToCurrentUser ? <div className="grid gap-2 border-t border-white/10 pt-4"><p className="text-sm text-white/80">审核决定</p><form action={approveAction} className="flex flex-wrap gap-2"><input name="id" type="hidden" value={review.id} /><Input className="min-w-56 flex-1 border-white/15 bg-black/25 text-white placeholder:text-white/35" name="comment" placeholder="通过说明（必填）" required /><Button className="bg-white text-black hover:bg-white/85" disabled={approving} type="submit">通过</Button></form><form action={changesAction} className="flex flex-wrap gap-2"><input name="id" type="hidden" value={review.id} /><Input className="min-w-56 flex-1 border-white/15 bg-black/25 text-white placeholder:text-white/35" name="comment" placeholder="退回修改说明（必填）" required /><Button variant="outline" className="border-amber-400/40 bg-transparent text-amber-200 hover:bg-amber-400/10 hover:text-amber-100" disabled={requesting} type="submit">要求修改</Button></form><form action={commentAction} className="flex flex-wrap gap-2"><input name="id" type="hidden" value={review.id} /><Input className="min-w-56 flex-1 border-white/15 bg-black/25 text-white placeholder:text-white/35" name="comment" placeholder="添加内部备注" required /><Button variant="outline" className="border-white/15 bg-transparent text-white hover:bg-white/10 hover:text-white" disabled={commenting} type="submit">记录备注</Button></form></div> : null}
    {canPublish && review.status === 'APPROVED' ? <form action={publishAction} className="flex flex-wrap items-center gap-3 border-t border-white/10 pt-4"><input name="contentId" type="hidden" value={review.content.id} /><p className="text-sm text-white/75">审核已通过，可发布为当前公开版本。</p><Button className="bg-white text-black hover:bg-white/85" disabled={publishing} type="submit">发布版本</Button></form> : null}
...
export function ReviewQueue({ reviews, reviewers, currentUserId, canPublish }: { reviews: Review[]; reviewers: Reviewer[]; currentUserId: string; canPublish: boolean }) {
  ...
  return <><div className="mt-6 flex flex-wrap gap-2">{([{ id: 'mine', label: '待我审核' }, { id: 'pending', label: '全部待审核' }, { id: 'handled', label: '我已处理' }, { id: 'overdue', label: '已超时' }] as const).map((item) => <Button className={filter === item.id ? 'bg-white text-black hover:bg-white/85' : 'border-white/15 bg-transparent text-white/65 hover:bg-white/10 hover:text-white'} key={item.id} onClick={() => setFilter(item.id)} type="button" variant={filter === item.id ? 'default' : 'outline'}>{item.label}</Button>)}</div>{filtered.length ? <div className="mt-4 grid gap-4">{filtered.map((review) => <ReviewCard canPublish={canPublish} currentUserId={currentUserId} key={review.id} review={review} reviewers={reviewers} />)}</div> : <div className="mt-4 rounded-2xl border border-dashed border-white/15 bg-white/[0.02] p-8 text-sm text-white/45">当前筛选条件下没有审核记录。</div>}</>;
}
