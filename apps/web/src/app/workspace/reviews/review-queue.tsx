...
import { reviewCardPermissions, type ReviewFilter } from './review-permissions';
...
function ReviewCard({ review, reviewers, currentUserId, canAssign, canPublish, filter }: { review: Review; reviewers: Reviewer[]; currentUserId: string; canAssign: boolean; canPublish: boolean; filter: ReviewFilter }) {
  ...
  const { canAssign: canAssignThisReview, canProcess: canProcessThisReview } = reviewCardPermissions({
    assignedReviewerId: review.assignedReviewer?.id ?? null,
    canAssign,
    currentUserId,
    filter,
  ...
  return <Card className="border border-white/[.1] bg-[#111112] py-5 shadow-none transition hover:border-white/[.18]"><CardHeader>
    <div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-[11px] tracking-[0.14em] text-white/40">{contentTypeLabel(review.content.contentType)} · v{review.version.versionNumber}</p><CardTitle className="mt-2 text-[19px] text-white">{review.version.title}</CardTitle><p className="mt-2 text-sm text-white/50">投稿人：{review.submittedBy.name} · {new Date(review.submittedAt).toLocaleString('zh-CN')}</p></div><WorkspaceStatusBadge status={review.status} /></div>
    ...
    <details className="border-t border-white/10 py-4"><summary className="cursor-pointer text-sm text-white/75">查看提交内容预览</summary><pre className="mt-3 max-h-96 overflow-auto whitespace-pre-wrap rounded-xl bg-black/25 p-4 text-xs leading-5 text-white/65">{displayValue(review.version.body)}</pre></details>
    <div className="border-t border-white/10 py-4"><form action={diffAction}><input name="id" type="hidden" value={review.id} /><Button variant="outline" className="h-9 rounded-[10px] border-white/[.14] bg-black/[.16] px-3 text-[12px] text-white/85 hover:bg-white/[.08] hover:text-white" disabled={loadingDiff} type="submit">{loadingDiff ? '加载差异…' : '查看版本差异'}</Button></form>{diffState.diff ? <div className="mt-4 grid gap-3 rounded-xl border border-white/10 bg-black/25 p-4"><p className="text-xs tracking-[0.14em] text-white/40">版本对比 · {diffState.diff.baseVersion ? `v${diffState.diff.baseVersion.versionNumber} → v${diffState.diff.version.versionNumber}` : `v${diffState.diff.version.versionNumber} 为首个版本`}</p>{diffState.diff.changes.length ? diffState.diff.changes.map((change) => <div className="grid gap-2 border-t border-white/10 pt-3 first:border-t-0 first:pt-0" key={change.path}><p className="text-sm font-medium text-white/85">{fieldName(change.path)}</p><div className="grid gap-2 md:grid-cols-2"><pre className="overflow-x-auto whitespace-pre-wrap rounded-lg bg-white/5 p-3 text-xs leading-5 text-white/55">{displayValue(change.before)}</pre><pre className="overflow-x-auto whitespace-pre-wrap rounded-lg bg-white/8 p-3 text-xs leading-5 text-white/80">{displayValue(change.after)}</pre></div></div>) : <p className="text-sm text-white/55">与基准版本没有字段差异。</p>}</div> : null}</div>
    <div className="flex flex-wrap items-center gap-2 border-t border-white/10 py-4"><span className="text-xs text-white/45">审核人：{review.assignedReviewer?.name ?? '暂未分配'}</span>{canAssignThisReview ? <form action={assignAction} className="flex flex-wrap items-center gap-2"><input name="id" type="hidden" value={review.id} /><NativeSelect className="h-9 rounded-[10px] border border-white/[.14] bg-black/[.16] px-3 text-[12px] text-white" defaultValue={review.assignedReviewer?.id ?? ''} name="reviewerId" required><option value="">选择审核人</option>{reviewers.filter((reviewer) => reviewer.id !== review.submittedBy.id).map((reviewer) => <option key={reviewer.id} value={reviewer.id}>{reviewer.name} · {reviewer.email}</option>)}</NativeSelect><Button variant="outline" className="h-9 rounded-[10px] border-white/[.14] bg-black/[.16] px-3 text-[12px] text-white/85 hover:bg-white/[.08] hover:text-white" disabled={assigning} type="submit">{review.assignedReviewer ? '重新分配' : '分配'}</Button></form> : null}</div>
    {canProcessThisReview ? <div className="grid gap-2 border-t border-[var(--v9-line)] pt-4"><form action={approveAction} className="flex flex-wrap gap-2"><input name="id" type="hidden" value={review.id} /><Input className="h-9 min-w-56 flex-1" name="comment" placeholder="通过说明（必填）" required /><Button className="h-9 rounded-[10px] px-3 text-[12px]" disabled={approving} type="submit">通过</Button></form><form action={changesAction} className="flex flex-wrap gap-2"><input name="id" type="hidden" value={review.id} /><Input className="h-9 min-w-56 flex-1" name="comment" placeholder="退回修改说明（必填）" required /><Button variant="outline" className="h-9 rounded-[10px] border-[var(--v9-status-warning-line)] bg-[var(--v9-status-warning-bg)] px-3 text-[12px] text-[var(--v9-status-warning-text)] hover:bg-[var(--v9-status-warning-bg)] hover:text-[var(--v9-status-warning-text)]" disabled={requesting} type="submit">要求修改</Button></form><form action={commentAction} className="flex flex-wrap gap-2"><input name="id" type="hidden" value={review.id} /><Input className="h-9 min-w-56 flex-1" name="comment" placeholder="添加内部备注" required /><Button variant="outline" className="h-9 rounded-[10px] px-3 text-[12px]" disabled={commenting} type="submit">记录备注</Button></form></div> : null}
...
export function ReviewQueue({ reviews, reviewers, currentUserId, canAssign, canPublish, initialFilter }: { reviews: Review[]; reviewers: Reviewer[]; currentUserId: string; canAssign: boolean; canPublish: boolean; initialFilter: ReviewFilter }) {
  ...
  return <><div className="mt-5 flex flex-wrap gap-2">{([{ id: 'mine', label: '待我审核' }, { id: 'pending', label: '全部待审核' }, { id: 'handled', label: '我已处理' }, { id: 'overdue', label: '已超时' }] as const).map((item) => <Button aria-pressed={filter === item.id} className={filter === item.id ? '' : 'text-[var(--v9-muted)]'} key={item.id} onClick={() => setFilter(item.id)} type="button" variant={filter === item.id ? 'default' : 'outline'}>{item.label}</Button>)}</div>{filtered.length ? <div className="mt-3 grid gap-3">{filtered.map((review) => <ReviewCard canAssign={canAssign} canPublish={canPublish} currentUserId={currentUserId} filter={filter} key={review.id} review={review} reviewers={reviewers} />)}</div> : <WorkspaceEmptyState className="mt-3">当前筛选条件下没有审核记录。</WorkspaceEmptyState>}</>;
}
