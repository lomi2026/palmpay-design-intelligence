      ...
      <section id="home" className="relative min-h-[648px] overflow-hidden px-3 py-16 lg:px-6">
        ...
        <div className="relative mx-auto grid max-w-[1232px] items-center gap-7 pt-[14px] lg:grid-cols-[510px_minmax(0,1fr)] lg:gap-[76px] lg:pt-[62px]">
          ...
          <div className="relative min-h-[520px] overflow-hidden rounded-[25px] border border-white/[.15] bg-[#0c0c0d] shadow-2xl shadow-black/40" aria-label="平台工作台预览">
            <div className="flex h-12 items-center border-b border-white/[.1] px-4 text-[10px] tracking-[.2em] text-white/45"><span className="mr-5 flex gap-1"><i className="size-1.5 rounded-full bg-white/35" /><i className="size-1.5 rounded-full bg-white/35" /><i className="size-1.5 rounded-full bg-white/35" /></span>设计能力工作台 <span className="ml-auto tracking-normal">● 测试运行</span></div>
            <div className="grid min-h-[468px] grid-cols-1 md:grid-cols-[142px_1fr]">
              <aside className="hidden border-r border-white/[.1] p-4 text-[10px] text-white/45 md:block"><div className="mb-7 flex items-center gap-2 font-semibold text-white"><span className="grid size-6 place-items-center rounded-md bg-white text-black">P</span>PalmPay UX</div><p className="mb-3 text-[8px] tracking-[.16em]">工作区</p>{['总览','设计资产','AI Skill','案例','','贡献','价值'].map((label) => label ? <p className={`mb-1 rounded-md px-2 py-2 ${label === '总览' ? 'bg-white/[.1] text-white' : ''}`} key={label}>▢&nbsp;&nbsp;{label}</p> : <div className="h-5" key="gap" />)}</aside>
              <div className="p-5"><h2 className="text-[16px] font-bold">设计能力总览</h2><p className="mt-1 text-[10px] text-white/45">团队资产、方法与验证实践</p><div className="mt-4 flex h-8 items-center rounded-lg border border-white/[.1] px-3 text-[10px] text-white/45"><Search className="mr-2 size-3" />搜索资产、Skill 或案例</div><div className="mt-3 grid grid-cols-3 gap-2">{['设计资产|24|已发布','AI Skill|12|已发布','有效复用|86|近 30 天'].map((item) => { const [label,value,meta] = item.split('|'); return <div key={label} className="rounded-xl border border-white/[.1] p-3"><p className="text-[8px] tracking-[.15em] text-white/45">{label}</p><strong className="mt-2 block text-[22px]">{value}</strong><em className="text-[8px] not-italic text-white/40">{meta}</em></div>; })}</div><div className="mt-4 text-[10px] font-semibold">精选内容 <span className="float-right text-white/40">查看全部 →</span></div><div className="mt-2 grid grid-cols-2 gap-2">{['Web 数据表格组件规范','UI 视觉走查 Skill'].map((title, index) => <div className="rounded-xl border border-white/[.1] p-3" key={title}><p className="text-[8px] tracking-[.13em] text-white/45">{index ? 'AI Skill' : '设计资产'}</p><strong className="mt-3 block text-[11px]">{title}</strong><p className="mt-1 text-[8px] text-white/40">统一复杂业务表格的布局、状态与交付标准。</p></div>)}</div><p className="mt-5 text-[10px] font-semibold">复用趋势 <span className="float-right text-white/40">近 12 周</span></p><div className="mt-4 h-px bg-gradient-to-r from-white/10 via-white/80 to-white/10" /></div>
            <div className="absolute bottom-1 -right-5 w-60 rounded-2xl border border-white/[.16] bg-[#111113] p-4 shadow-2xl shadow-black/50"><p className="text-[8px] tracking-[.2em] text-white/45">已验证内容覆盖率</p><strong className="mt-2 block text-[13px] leading-5">60% 已发布内容完成验证</strong><div className="mt-3 h-1 rounded-full bg-white/[.09]"><div className="h-full w-3/5 rounded-full bg-[#f4f4f4]" /></div></div>
          </div>
          </div>
        </div>
      </section>
      ...
      <section id="skills" className="px-5 py-[58px] md:px-6 md:py-[72px]">
        <div className="mx-auto max-w-[1232px]">
          <div className="mb-10 max-w-[560px]">
            <h2 className="text-[34px] font-semibold tracking-[-.05em] text-white md:text-[42px]">高频入口，直达工作台</h2>
      ...
      <section id="projects" className="px-5 pb-10 pt-[72px] md:px-6 md:pt-[96px]">
        <div className="mx-auto max-w-[1232px]">
          <div className="mb-10 flex flex-wrap items-end justify-between gap-6">
            <div>
              <h2 className="text-[34px] font-semibold tracking-[-.05em] text-white md:text-[42px]">33 个 AI 体验设计探索项目</h2>
