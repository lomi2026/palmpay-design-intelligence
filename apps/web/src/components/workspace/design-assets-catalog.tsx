...
function AssetCover({ category, cover, platform }: { category: string; cover?: string; platform: string }) {
  const preview = (() => {
    switch (cover) {
      ...
      case 'compare': return <div className="absolute inset-x-5 top-3 grid h-28 grid-cols-2 gap-2"><div className="rounded-xl border border-white/15 bg-black/95 p-3 shadow-xl"><div className="mb-2 text-[9px] tracking-widest text-white/40">设计</div><div className="h-16 rounded-md border border-white/20 p-2"><div className="h-2 w-12 rounded bg-white/20" /><div className="mt-3 h-6 rounded bg-white" /></div></div><div className="relative rounded-xl border border-white/15 bg-black/95 p-3 shadow-xl"><div className="mb-2 text-[9px] tracking-widest text-white/40">实现</div><div className="h-16 rounded-md border border-white/20 p-2"><div className="h-2 w-10 rounded bg-white/20" /><div className="mt-3 h-6 rounded border-2 border-dashed border-white/70" /></div><span className="absolute top-12 right-1 grid size-5 place-items-center rounded-full bg-white text-[9px] text-black">3</span></div></div>;
      case 'table':
  ...
  return (
    <section>
      ...
      {filtered.length ? (
              ...
              <Card className={cn('gap-0 overflow-hidden border border-white/10 bg-white/[0.025] py-0 shadow-none transition hover:-translate-y-0.5 hover:border-white/20 hover:bg-white/[0.045]', view === 'list' && 'md:grid md:grid-cols-[minmax(0,1fr)_auto] md:items-center')} key={content.id}>
                ...
                <CardHeader className="p-4 pb-4">
                  <div className="flex items-center justify-between gap-3 text-xs">
                    {view === 'list' ? <div className="flex min-w-0 items-center gap-1.5 text-[var(--v9-subtle)]"><span className="truncate">{content.category?.name ?? '未分类'}</span><span aria-hidden="true">·</span><span>{content.assetDetail?.platforms[0] ?? '全平台'}</span></div> : <WorkspaceStatusBadge status={status} />}
        ...
        <WorkspaceEmptyState className="mt-3.5 py-16 text-center">没有符合当前筛选条件的资产。</WorkspaceEmptyState>
      )}
    </section>
  );
}
