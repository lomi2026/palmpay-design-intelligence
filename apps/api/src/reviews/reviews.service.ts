...
export class ReviewsService {
  ...
  async submit(user: AuthenticatedUser, contentId: string, input: SubmitReviewDto) {
    ...
    await this.engagement.recordEvent(user, 'content_submit', content.id, {
  ...
  async assign(user: AuthenticatedUser, reviewId: string, input: AssignReviewerDto) {
    ...
    if (review.submittedById === reviewer.id)
      throw new ConflictException('A submitter cannot review their own content.');
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      await tx.reviewAction.create({
        data: {
          ...
          actorId: user.id,
          action: ReviewActionType.ASSIGN,
  ...
  async comment(user: AuthenticatedUser, reviewId: string, input: ReviewDecisionDto) {
    ...
    return this.prisma.reviewAction.create({
      data: {
        reviewRequestId: review.id,
        actorId: user.id,
        action: ReviewActionType.COMMENT,
  ...
  async queue(user: AuthenticatedUser) {
    ...
    const now = new Date();
    return {
      items: items.map((item) => ({ ...item, isOverdue: Boolean(item.dueAt && item.dueAt < now) })),
    };
  }
  ...
  async mine(user: AuthenticatedUser) {
    const items = await this.prisma.reviewRequest.findMany({
      ...
      include: {
        ...
        version: {
          select: { versionNumber: true, title: true, versionStatus: true, changeSummary: true },
        },
        assignedReviewer: { select: { id: true, name: true, email: true } },
        actions: {
          orderBy: { createdAt: 'asc' },
  ...
  async diff(user: AuthenticatedUser, reviewId: string) {
    ...
    return {
      baseVersion: baseVersion
        ? { id: baseVersion.id, versionNumber: baseVersion.versionNumber }
        : null,
  ...
  private async finishReview(
    ...
    const completed = await this.prisma.$transaction(async (tx) => {
      ...
      if (!(
        content.status === ContentStatus.PUBLISHED && content.currentVersionId !== review.versionId
      )) {
        await tx.content.update({
          where: { id: review.contentId },
      ...
      await tx.contentVersion.update({
        ...
        data: { versionStatus: contentStatus },
      });
      await tx.reviewAction.create({
        data: { reviewRequestId: review.id, actorId: user.id, action, comment },
      });
      await tx.notification.create({
        data: {
          receiverId: completed.submittedById,
          type:
    ...
    await this.audit.write({
      organizationId: user.organizationId,
      actorId: user.id,
      action: action === ReviewActionType.APPROVE ? 'review.approve' : 'review.request_changes',
      entityType: 'review_request',
      entityId: review.id,
      afterData: { status: reviewStatus, contentStatus },
    });
    await this.engagement.recordEvent(
      user,
      action === ReviewActionType.APPROVE ? 'review_approve' : 'review_reject',
      review.contentId,
      { reviewId: review.id },
    );
    return completed;
  ...
  private async findAssignedReview(user: AuthenticatedUser, reviewId: string) {
    const review = await this.findProcessableReview(user, reviewId);
    if (review.assignedReviewerId !== user.id)
      throw new ForbiddenException('This review is not assigned to you.');
    if (review.submittedById === user.id)
  ...
  private diffValues(
    ...
    if (this.isPlainObject(before) && this.isPlainObject(after)) {
      ...
      return keys.flatMap((key) =>
        this.diffValues(before[key], after[key], path ? `${path}.${key}` : key),
      );
    }
    return [{ path, before, after }];
  }
